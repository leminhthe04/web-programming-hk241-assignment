...
export default function TransactionHist() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const fetchUserData = axios.get(`http://localhost/Assignment/Backend/api/user/detail/${userID}`);
                const fetchHistoryData = axios.get(`http://localhost/Assignment/Backend/api/user/history/${userID}/fetch/0/10`);

                const [userDataResponse, historyDataResponse] = await Promise.all([fetchUserData, fetchHistoryData]);

                if (userDataResponse.status === 200) {
                    console.log("Check user data: ", userDataResponse.data.data[0])

                    setUserDetail(userDataResponse.data.data[0]);
                }
